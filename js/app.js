(() => {
  ...
  const state = {
    ...
    places: [], filtered: [], shown: PAGE, markers: [], savedOnly: false, mapMoved: false,
  };
  ...
  function applyFilter() {
    ...
    if (q) rows = rows.filter((p) => p.name.toLowerCase().includes(q) || p.cuisine.toLowerCase().includes(q));
  ...
  function renderList() {
    ...
    listEl.querySelectorAll(".heart").forEach((btn) => {
      btn.addEventListener("click", () => { toggleFav(btn.dataset.id); renderList(); });
    });
    moreBtn.classList.toggle("hidden", state.filtered.length <= state.shown);
    setStatus(rows.length + " of " + state.filtered.length + (state.savedOnly ? " saved" : " nearby"));
  ...
  async function fetchPlaces(lat, lon, miles) {
    ...
    for (const el of data.elements || []) {
      ...
      places.push({
        id, name, lat: lat2, lon: lon2, miles: haversine(lat, lon, lat2, lon2),
  ...
  async function geocodeCity(q) {
    ...
    return { lat: Number(data[0].lat), lon: Number(data[0].lon), label: data[0].display_name.split(",").slice(0, 2).join(",") };
  }
  async function loadAt(lat, lon, label) {
    state.lat = lat; state.lon = lon; state.label = label || ""; state.mapMoved = false;
  ...
  function useLocation() {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      () => { setStatus("Location blocked. Enter a city to browse."); cityInput.focus(); },
  ...
  radiusInput.addEventListener("input", () => { radiusLabel.textContent = radiusInput.value + " mi"; });
  ...
  $("search-form").addEventListener("submit", async (e) => {
    ...
    } catch { setStatus("Could not find that place."); }
  });
